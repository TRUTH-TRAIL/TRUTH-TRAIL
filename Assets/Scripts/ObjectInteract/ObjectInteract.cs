...
public class ObjectInteract : MonoBehaviour
{
    ...
    private void ClueUpdate(GameObject clue)
    {
        ...
        //20% È®·ü·Î ÀúÁÖ. 80%È®·ü·Î °¡Â¥ + ÁøÂ¥
        if (RandomInt!=0 || curse.activeCurse)
        {
            //¸Þ¸ð¿¡ Ç¥ÇöµÇ´Â °³¼ö 10°³ ÃÊ°ú ¸øÇÔ
            if(clueTextIndex<10){
                //clueTextIndex ÀÌÀü ºñ¾îÀÖ´Â Ä­ ÀÖÀ¸¸é ±×°÷¿¡ Ã¤¿ò
                ...
                Debug.Log("¾çÃÊ¸¦ »ç¿ëÇØ °¡Â¥ ÀúÁÖ¸¦ ¾ø¾Ö¼¼¿ä."); //ÀÌ ºÎºÐÀº ÃßÈÄ ±âÈ¹¿¡ ¸Â°Ô °íÃÄ¾ßÇÔ.
            }
    ...
    private void HandleCandle(Transform target)
    {
        ...
        if(trueKeyCount>=10&&candleNum<3){
            ...
            if(Player.transform.GetChild(1).GetChild(5).gameObject.activeSelf && newClue)
            {
                ...
                {
                    //ÇãÇã ÀÌÆåÆ® Àû¿ëÇß¾ú´Âµ¥ PlayerÀÇ getchild(3) ¾îµð°¬¾î?????????
